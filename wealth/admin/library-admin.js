...
function renderLibrary() {
  ...
  $("memberAdminBody").innerHTML = visible.map((member) => {
    ...
    return `<tr><td><div class="cell-stack"><strong>${escapeHtml(member.name || "未命名会员")}</strong><span>${escapeHtml(member.email)}</span></div></td><td><span class="admin-badge ${member.google_linked ? "published" : "draft"}">${member.google_linked ? "已绑定" : "等待首次登录"}</span></td><td>${labelLevel(member.tier)}</td><td>${escapeHtml(expiry)}</td><td><span class="admin-badge ${active ? "published" : "ended"}">${active ? "有效" : expired(member) ? "已到期" : "已暂停"}</span></td><td><div class="row-actions"><button data-member-edit="${escapeHtml(member.id)}">编辑</button><button data-member-revoke="${escapeHtml(member.id)}">强制下线</button><button class="delete-button" data-member-delete="${escapeHtml(member.id)}">删除</button></div></td></tr>`;
  }).join("");
}
...
function openMember(item = null) {
  ...
  $("memberEmail").value = item?.email || ""; $("memberName").value = item?.name || ""; $("memberTier").value = item?.tier || "basic"; $("memberStatusInput").value = item?.status || "active"; $("memberStartsAt").value = dateInput(item?.starts_at) || new Date().toISOString().slice(0, 10); $("memberExpiresAt").value = dateInput(item?.expires_at); $("memberNotes").value = item?.notes || ""; $("memberDialog").showModal();
}
