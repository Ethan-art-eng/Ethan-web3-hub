...
export async function getMember(env, email) {
  ...
  const notStarted = Boolean(member.starts_at && Date.parse(member.starts_at) > Date.now());
  return { ...member, allowed: member.status === "active" && !expired && !notStarted, expired, notStarted };
}
...
export async function listLibrary(env) {
  ...
  const [articles, courses, lessons, members] = await Promise.all([
    ...
    db.prepare("SELECT id, email, name, tier, status, starts_at, expires_at, notes, created_at, updated_at, CASE WHEN google_sub IS NOT NULL AND google_sub != '' THEN 1 ELSE 0 END AS google_linked FROM members ORDER BY updated_at DESC").all(),
  ]);
...
export async function upsertLesson(db, input) {
  ...
  await db.prepare(`INSERT INTO lessons (id, course_id, title, description, stream_uid, duration_minutes, status, access_level, sort_order, created_at, updated_at)
    ...
    .bind(id, courseId, title, cleanText(input.description, 800), cleanText(input.stream_uid, 1500), Math.max(0, Number(input.duration_minutes) || 0),
...
export async function upsertMember(db, input) {
  ...
  const codeHash = existing?.access_code_hash || "";
  const status = input.status === "paused" ? "paused" : "active";
  const write = db.prepare(`INSERT INTO members (id, email, name, tier, status, starts_at, expires_at, notes, created_at, updated_at, access_code_hash)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    access_code_hash=members.access_code_hash, google_sub=CASE WHEN members.email != excluded.email THEN NULL ELSE members.google_sub END`)
    .bind(id, email, cleanText(input.name, 120), input.tier === "premium" ? "premium" : "basic", status,
      cleanText(input.starts_at, 40) || now, expiry, cleanText(input.notes, 800), cleanText(input.created_at, 40) || now, now, codeHash);
  const mustRevokeSessions = Boolean(status === "paused" || existing?.email !== email || (expiry && Date.parse(expiry) <= Date.now()));
  if (mustRevokeSessions) {
    const emails = [...new Set([existing?.email, email].filter(Boolean))];
    await db.batch([write, ...emails.map((value) => db.prepare("DELETE FROM member_sessions WHERE email = ?").bind(value))]);
  } else {
    await write.run();
  }
  return id;
