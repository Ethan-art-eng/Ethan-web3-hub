...
export async function upsertMember(db, input) {
  ...
  if (!existing && code.length < 12) throw new Error("新会员需要设置至少 12 位会员码");
  if (existing && existing.email !== email && code.length < 12) throw new Error("修改会员邮箱时需要同时重置至少 12 位会员码");
  if (code && code.length < 12) throw new Error("新会员码至少需要 12 位");
  ...
  const mustRevokeSessions = Boolean(code || status === "paused" || existing?.email !== email || (expiry && Date.parse(expiry) <= Date.now()));
