...
function entryPointLabel(value) {
  return value === "wallet" ? "交易所钱包" : "交易所主站";
}
...
function normalizeCampaign(item, syncedAt) {
  ...
  return {
    ...
    venue: `${meta.shortName} · ${entryPointLabel(item.entry_point)}`,
...
export function normalizeBarkerDataset(rawCampaigns, syncedAt = new Date().toISOString()) {
  ...
  return {
    updatedAt: syncedAt,
    notice: "活动数据自动同步自 Barker，仅保留交易所理财活动。APY、额度、地区资格和活动状态可能变化；参与前请在交易所官方页面再次确认。",
    dataProvider: "Barker",
